import{u as d,V as r,C as u,f as b,a as g}from"./index-BVinSk0X.js";import{C as f,W as h,S as w,a as k,O as C,F as v,R as y,i as x}from"./index-Cd9XZf6e.js";const a=new f,$=a.get(h),t=$.create();t.scene=new w(a);t.scene.setup();t.scene.three.background=null;const m=document.getElementById("container");t.renderer=new k(a,m);t.camera=new C(a);await t.camera.controls.setLookAt(68,23,-8.5,21.5,-5.5,23);a.init();const L="https://thatopen.github.io/engine_fragment/resources/worker.mjs",P=await fetch(L),S=await P.blob(),U=new File([S],"worker.mjs",{type:"text/javascript"}),j=URL.createObjectURL(U),o=a.get(v);o.init(j);t.camera.controls.addEventListener("rest",()=>o.core.update(!0));t.onCameraChanged.add(e=>{for(const[,i]of o.list)i.useCamera(e.three);o.core.update(!0)});o.list.onItemSet.add(({value:e})=>{e.useCamera(t.camera.three),t.scene.three.add(e.object),o.core.update(!0)});const B=["/resources/frags/school_arq.frag"];await Promise.all(B.map(async e=>{var c;const i=(c=e.split("/").pop())==null?void 0:c.split(".").shift();if(!i)return null;const p=await(await fetch(e)).arrayBuffer();return o.core.load(p,{modelId:i})}));const D=a.get(y);D.get(t);const n=a.get(x);n.enabled=!0;m.ondblclick=()=>{n.enabled&&n.create(t)};const R=()=>{for(const[,e]of n.list)e.enabled=!e.enabled};window.onkeydown=e=>{(e.code==="Delete"||e.code==="Backspace")&&n.enabled&&n.delete(t)};d.init();const s=r.create(()=>b`

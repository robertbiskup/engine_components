import{C as g,a as f}from"./index-WipdELyR.js";import{l as v,L as p,m}from"./index-CimbCcJO.js";import{C as h,W as w,S as y,a as C,O as L,F as S}from"./index-B3o0n-Ei.js";import{G as k}from"./index-oDbgpygJ.js";const r=new h,G=r.get(w),t=G.create();t.scene=new y(r);t.scene.setup();t.scene.three.background=null;const I=document.getElementById("container");t.renderer=new C(r,I);t.camera=new L(r);await t.camera.controls.setLookAt(68,23,-8.5,21.5,-5.5,23);r.init();const U="https://thatopen.github.io/engine_fragment/resources/worker.mjs",$=await fetch(U),x=await $.blob(),E=new File([x],"worker.mjs",{type:"text/javascript"}),N=URL.createObjectURL(E),a=r.get(S);a.init(N);t.camera.controls.addEventListener("rest",()=>a.core.update(!0));t.onCameraChanged.add(n=>{for(const[,e]of a.list)e.useCamera(n.three);a.core.update(!0)});a.list.onItemSet.add(({value:n})=>{n.useCamera(t.camera.three),t.scene.three.add(n.object),a.core.update(!0)});const j=["https://thatopen.github.io/engine_components/resources/frags/school_arq.frag"],[d]=await Promise.all(j.map(async n=>{var u;const e=(u=n.split("/").pop())==null?void 0:u.split(".").shift();if(!e)return null;const o=await(await fetch(n)).arrayBuffer();return a.core.load(o,{modelId:e})})),O=r.get(k),l=O.create(t),z=await d.getItemsOfCategories([/BUILDINGSTOREY/]),B=Object.values(z).flat(),b=await d.getItemsData(B),F=async n=>{const e=b.find(o=>"Name"in o&&"value"in o.Name?o.Name.value===n:!1);if(!e||!("Elevation"in e&&"value"in e.Elevation))return 0;const[,s]=await d.getCoordinates();return e.Elevation.value+s};v.init();const c=p.create(()=>m`
    <bim-panel active label="Grids Tutorial" class="options-menu">
      <bim-panel-section label="Section">
        <bim-dropdown @change=${async({target:e})=>{const[s]=e.value;if(!s)return;const o=await F(s);l.three.position.y=o}} placeholder="Select a grid level">
  ...
  `);document.body.append(c);const P=p.create(()=>m`
